...
const EmoteUtil = require('./emoteUtils.js');//<<<<<< This definiatly needs to be put into the utils... probably... but for now it "SHOULD" be ok?
 ...
 * @param {MessageUtils} utils
 */
function cleanEmotesOnly(utils){
	if(utils.settingsManager.getEmoteOnlyMode())
	{
		var mssg = utils.getMessage();
		if(/^!/.test(mssg) || utils.botUtils.checkRole(utils.getUser(), 'manager'))
		{
			return;
		}
		else
		{
			//send a check to see if we have emotes
			if(!EmoteUtil.isOnlyEmotes(utils,mssg))
			{
				//we remove the mssg
...
function cleanChatAntiCaps(utils) {
    ...
    if(upper > 10 && (upper / (upper + lower)) > 0.5) {
        ...
        utils.botUtils.timeMute(
            ...
            5,
            'using a lot of caps',
            () => console.logPunishment(`Message: "${utils.getMessage()}" (${`${(upper / (upper + lower)) * 100}% uppercase`})`)
        );
        utils.reply.stop();
    }
}
            ...
            if(typeof phrases[key] != 'function') {
                ...
                for(var i = 0; i < phrasesList.length; i++) {
                    ...
                    if(checkIfContains(phrase)) {
                        ...
                        const banCallback = () => console.logPunishment(`Message: "${utils.getMessage()}"`);
                        if(isPermanent)
    ...
    if(sameThreadImagesCount === parseInt(utils.settingsManager.getImgRemoveSpamAmount())) {
        ...
        if(!utils.botUtils.checkRole(utils.getUser(), 'resident-dj'))
            utils.botUtils.timeMute(
                ...
                `sending more than ${utils.settingsManager.getImgRemoveSpamAmount()} images in a row`
            );
...
function cleanChatImageTimeout(utils) {
    ...
    if(
        ...
        utils.botUtils.timeMute(
            ...
            `sending an image/images having less than ${utils.strings.formatMeasure(utils.settingsManager.getImgDubsAmount(), 'dub')}`
        );
        utils.reply.stop();
    }
